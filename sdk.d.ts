...
export declare namespace MpSdk {
	export { App, Camera, Conversion, Floor, Label, Mattertag, Measurements, Mode, Model, Pointer, Renderer, Room, Sensor, Settings, Sweep, Tour, };
}
/**
 * A Window type that can be use to cast the window embedding Showcase's iframe to hint at the existance of the `MP_SDK`.
 * ```
 * const embedIframe = document.getElementById('showcase') as HTMLIFrameElement;
 * (window as ShowcaseEmbedWindow).MP_SDK.connect(embedIframe, 'my-sdk-key');
 * ```
 */
export declare type ShowcaseEmbedWindow = Window & typeof globalThis & {
	MP_SDK: {
		connect(target: HTMLIFrameElement, sdkKey: string, unused: string): Promise<MpSdk>;
	};
};

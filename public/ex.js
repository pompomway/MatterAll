...
document.addEventListener("DOMContentLoaded", () => {
    //querySelector()方法用於返回與文檔中指定的CSS選擇器匹配的第一個元素。
    //以下取得Html所有元件
    iframe = document.querySelector('.showcase');
    container = document.querySelector('.showcase_container');　　//iframeのdiv class(sid-inputも含め)
    ...
    sidSelector = document.getElementById('sid-input');  //画面右上sid input
    ...
    sidSelector.addEventListener('keyup', e => {
        if(e.key === "Enter"){       //Enter押すと、Sidを変更する
...
function addToTable(tag){ //把tag加進下方table
    let elem;
    let row;
...
function loadedShowcaseHandler(mpSdk){  //Showcase讀取
    let addingTag = false;
    let movingTag = false;
    // Fetch tags
    ...
    function updateTagPos(newPos, newNorm=undefined, scale=undefined){ //intersectionData進來，呼叫editPosition修改
        if(!newPos) return;
        if(!scale) scale = .33;
    ...
    mpSdk.Pointer.intersection.subscribe(intersectionData => { //移動的話變更整個intersectionData
    ...
    function replaceShowcaseTags(tags){  //新增tags
        return mpSdk.Mattertag.getData()
        .then(oldTags => {   //frame現有的tag刪除
            oldTagSids = oldTags.map(oldTag => oldTag.sid);
            return mpSdk.Mattertag.remove(oldTagSids);
        })
        .then( () => {  //新載進來的tags(引数)，對其中的media處理
            tags.forEach(tag => {
        ...
        .then(newSids => {
            //tags.forEach( (tag, i) => tag.sid = newSids[i]); //給新ID?但沒有這行ID還是一直更新
    ...
    importBtn.addEventListener('click', () => {
        importJson();
        /*原import
        const input = document.createElement('input');  //建立 HTML 元素叫做input
        input.type = 'file';  //input類型為file//實際動作為開啟選取本地file視窗
        let file;
        input.onchange = e => {  //onchange離開選定後該元件觸發事件，例如:input欄位輸入,離開該欄位時,觸發小寫字母轉大小//此為選取檔案後觸發下方事件
            file = e.target.files[0];//選取第一份檔案
            importFile(file);  //將檔案丟進importFile
    ...
    async function importJson(){
        ...
        .then(res => {
           ...
           //console.log(result);
           replaceShowcaseTags(result).then((newTags) => {
    ...
    function importFile(file){ //原import
    ...
    function removeAllTags(){
        mpSdk.Mattertag.getData()
        .then(tags => {  //每個tag都歷一遍了為什麼還要多一步把sid取出來?
    ...
    function exportTags(tags){ //原export
        if(!tags || tags.length == 0){return;} // TODO: Let the user know there are no tags
        const filename = 'tags.json'; //建立file
        const tagsText = JSON.stringify(tags); //把tags轉成stringify
        download(tagsText, filename, "application/json"); //把資料以json格式存入，並讓瀏覽器下載
    }
